...
void CallGraph::expandFunctions() {
    // Backup graph edges
    unordered_map<NodePtr, set<NodePtr>> childrenBackup;
    for (auto it : nodes) {
        childrenBackup[it.second] = it.second->callsFunctions;
    }

    for (auto it : nodes) {
        NodePtr n = it.second;

        // Add all grandchildren as normal children
        if (n->id != rootId) {
            // Don't expand "main(void)" (design decision described in report)

            for (NodePtr child : childrenBackup[n]) {
                for (NodePtr grandChild : childrenBackup[child]) {
                    // Don't worry about duplicates since they're sets
                    n->callsFunctions.insert(grandChild);
                    grandChild->calledByFunctions.insert(n);
                }
            }
        }
    }
